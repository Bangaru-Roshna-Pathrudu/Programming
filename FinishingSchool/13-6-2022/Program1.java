...
Manoj must take the wfollowing actions:
- At Step-i, he must awaken the order[i]-th person, where 0 <= i < N
- If no one is sleeping between the people in the 0-th and right most awake person, 
...
class Solution{
    public static void main(String[] args){
        ...
        System.out.println(songPlayCount(wake,n));
    }
    
    static int songPlayCount(int[] wake,int n){
        int wokesum = 0,max = 0,count = 0;
        for(int i=0;i<n;i++){
            max = Math.max(max,wake[i]*(wake[i]+1)/2);
            wokesum += wake[i];
            if(wokesum == max) count++;
        }
        return count;
    }
}
