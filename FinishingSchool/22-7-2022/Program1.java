...
Your task is to find the number of unique Card Castles you can build using all N 
cards. 
...
class Solution{
    public static void main(String[] args){
        ...
        int n = sc.nextInt();
        System.out.println(getCastleCount(n));
    }
    
    static int getCastleCount(int n){
        int memo[] = new int[n+1];
        memo[0] = 1;
        for(int i=1;i<=(n/3)+1;i++){
            int cardsUsed = (3*i)-1;
            for(int j=n;j>=cardsUsed;j--){
                memo[j] += memo[j-cardsUsed];
            }
        }
        return memo[n];
    }
