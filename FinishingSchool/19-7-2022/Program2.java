...
class Solution{
    public static void main(String[] args){
        ...
        for(int i=0;i<p.length;i++) pid[i] = Integer.parseInt(p[i]);
        System.out.println(getTicketCount(pid));
    }
    
    static int getTicketCount(int[] pid){
        Map<Integer,Integer> map = new HashMap<>();
        int l = pid.length,tic = 0;
        for(int i=0;i<l;i++){
            map.put(pid[i],map.getOrDefault(pid[i],0)+1);
        }
