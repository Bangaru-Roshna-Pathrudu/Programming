...
class Solution{
    public static void main(String[] args){
        ...
        int front[] = new int[n];
        int back[] = new int[n];
        for(int i=0;i<n;i++) front[i] = s.nextInt();
        for(int i=0;i<n;i++) back[i] = s.nextInt();
        System.out.println(getWinner(front,back,n));
    }
    
    static int getWinner(int[] front,int[] back,int n){
        Set<Integer> noChance = new HashSet<>();
        for(int i=0;i<n;i++){
            if(front[i] == back[i]) noChance.add(front[i]);
        }
        int winner = Integer.MAX_VALUE;
        for(int i=0;i<n;i++){
            if(!noChance.contains(front[i]))
                winner = Math.min(winner,front[i]);
        }
        for(int i=0;i<n;i++){
            if(!noChance.contains(back[i]))
                winner = Math.min(winner,back[i]);
        }
        return (winner == Integer.MAX_VALUE)?(0):(winner);
    }
}
