...
class Solution {
    ...
    public int numEqualToSumOfDescendants(BinaryTreeNode root) {
       ...
       int sum = getDescendantSum(root);
       return count;
    }
    
    static int getDescendantSum(BinaryTreeNode root){
        if(root == null || root.data == -1) return 0;
        int l = getDescendantSum(root.left);
        int r = getDescendantSum(root.right);
        if(l+r == root.data) count++;
